...
function detect_packet_manager() {
    ...
    PACKET_MANAGERS=(
        "Debian|screen:ffmpeg:libnice10|apt:dpkg-query||"
        "Ubuntu|screen:ffmpeg:libnice10|apt:dpkg-query||"
        "openSUSE|screen:ffmpeg:libnice10|yzpper:rpm||"
        "CentOS|screen:ffmpeg:libnice:yum-utils|yum:rpm|${CentOS_6_REPO}|6"
        "CentOS|screen:ffmpeg:libnice:yum-utils|yum:rpm|${CentOS_7_REPO}|7"
        "RedHat|screen:ffmpeg:libnice:yum-utils|yum:rpm||"
        "Arch|screen:ffmpeg:libnice|pacman||"
        "Fedora|screen:ffmpeg:libnice|dnf:rpm|${FEDORA_REPO}|"
        "Mint|screen:ffmpeg:libnice10|apt:dpkg-query||"
    )
    ...
    do
        ...
        cyan " "
        green "${SYSTEM_NAME_DETECTED} ${SYSTEM_VERSION} detected!"
        debug "Found system ${data[0]} ${SYSTEM_VERSION}"
...
if [ "${NEDDED_REPO}" == "true" ] && ! yum -v repolist all 2>/dev/null | grep "epel-multimedia" &>/dev/null && ! dnf -v repolist all | grep "fedora-multimedia.repo" &>/dev/null; then
    cyan " "
    ...
    if [ "$OPTION" == "${OPTIONS[0]}" ]; then
        ...
        for i in ${data[3]};
        do
            if [ "$SYSTEM_NAME_DETECTED" == "CentOS" ]; then
...
YTDL_REQUEST_URL="https://yt-dl.org/downloads/latest/youtube-dl"
YTDL_PATH="/usr/local/bin/youtube-dl"
if ! [ -x "$(command -v youtube-dl)" ]; then
    cyan " "
    cyan "Should we download "youtube-dl" for you? (Required root or administrator privileges)"
    OPTIONS=("Yes" "No" "Quit")
    select OPTION in "${OPTIONS[@]}"; do
        case "$REPLY" in
            1|2) break;;
    ...
    if [ "$OPTION" == "${OPTIONS[0]}" ]; then
        green_sleep "# Downloading ${YTDL_REQUEST_URL} to ${YTDL_PATH}"
        ${SUDO_PREFIX} curl -s --connect-timeout 10 -S -L "$YTDL_REQUEST_URL" -o "$YTDL_PATH"
        if [ $? -ne 0 ]; then
            error "Failed to download youtube-dl!"
            exit 1
        fi
        green_sleep "DONE!"

        cyan " "
        green_sleep "# Making youtube-dl usable..."
        ${SUDO_PREFIX} chmod a+rx ${YTDL_PATH}
        cyan "Saved under: ${YTDL_PATH}"
        green_sleep "DONE!"
    fi
fi
...
TEASPEAK_VERSION=$(curl -s --connect-timeout 10 -S -L -k https://repo.teaspeak.de/latest)
TEA_REQUEST_URL="https://repo.teaspeak.de/server/linux/x64/TeaSpeak-${TEASPEAK_VERSION}.tar.gz"
if [ $? -ne 0 ]; then
...
${SUDO_PREFIX} chmod 774 /$teaPath/$teaUser/*.sh
green_sleep "DONE!"
